...
import { vendorSideLink } from "../../config/vendor.site";
  ...
  return (
    <div className={`min-h-screen ${isDark ? "bg-[#141312] text-[#F9F5F0]" : "bg-[#F9F5F0] text-[#3D352F]"}`}>
      ...
      <div className="lg:hidden flex items-center justify-between px-4 py-3 bg-base-100 border-b border-base-200 sticky top-16 z-20">
        ...
        <span className="badge badge-secondary badge-sm font-semibold">VENDOR HUB</span>
      </div>
      ...
      <div className="flex w-full">
        ...
        >
          {/* Header */}
          <div className="flex items-center gap-3 px-3 py-3 mb-6 rounded-2xl bg-secondary/10 text-secondary">
            <div className="w-10 h-10 rounded-xl bg-secondary text-neutral flex items-center justify-center font-bold">
              <Store size={20} />
            </div>
            <div>
              <h3 className="font-bold text-sm leading-tight text-base-content">Vendor Hub</h3>
              <p className="text-[11px] text-gray-500 font-medium">Merchant Portal</p>
            </div>
          </div>

          {/* Navigation Links */}
          <nav className="space-y-2 flex-1">
            {vendorSideLink.map((route, index) => (
              <NavLink
                key={index}
                to={route.path}
                className={({ isActive }) =>
            ...
            >
              <div className="flex items-center justify-between pb-4 mb-4 border-b border-base-200">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-base">Merchant Navigation</span>
                </div>
                <button
                  onClick={() => setIsMobileOpen(false)}
